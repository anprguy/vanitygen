...
def worker(prefix, addr_type, result_queue, stop_event, stats_counter):
...
class CPUGenerator:
    def __init__(self, prefix, addr_type='p2pkh', cores=None):
        self.prefix = prefix
    ...
    def start(self):
        for _ in range(self.cores):
            p = multiprocessing.Process(target=worker, args=(
                self.prefix, self.addr_type, self.result_queue, self.stop_event, self.stats_counter
            ))
            p.start()
